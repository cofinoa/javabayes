...
public boolean mouseUp(Event evt, int x, int y) {
   ...
   public void paint(Graphics g) {
    	...
    	for (e = ig.elements(); e.hasMoreElements(); ) {
    	    ...
    	    for (ee = (node.get_parents()).elements(); ee.hasMoreElements(); ) {
        		...
        		//if node or parent are in the view draw the arc
        		if ((x1 >= 0-NODE_RADIUS && x1<=w+NODE_RADIUS 
           	      && y1 >= 0-NODE_RADIUS && y1<=h+NODE_RADIUS) ||
           	        (x2 >= 0-NODE_RADIUS && x2<=w+NODE_RADIUS 
        	      && y2 >= 0-NODE_RADIUS && y2<=h+NODE_RADIUS) 
           	    ) {
            ...
            // Draw the nodes if it is in the view
	    	if (x1 >= 0-NODE_RADIUS && x1<=w+NODE_RADIUS 
	         && y1 >= 0-NODE_RADIUS && y1<=h+NODE_RADIUS ){
	    		nn++;
	    		// Draw node's border
	    	    g.setColor(nodeBorderColor);
        	    g.fillOval( x1 - NODE_RADIUS - 1,
	    	                    y1 - NODE_RADIUS - 1,
	        	...
	        	//Draw node's name
	 	    	...
	 	    	g.drawString(node.get_name(),
	 	    	             x1 - SPACE_DRAW_NODE_NAME,
	 	    	             y1 + SPACE_DRAW_NODE_NAME);
	    	    }
        }
    	...
    	/*
    	System.out.println("time to call paint method: "+(t2-t1)+" ms ("+nn+" nodes & "+na+" arcs)");
    	System.out.println("[x_min="+x_min+" x_max="+x_max+" y_min="+y_min+" y_max="+y_max+"]");
    	System.out.println("[x_scroll="+x_scroll+" y_scroll="+y_scroll+"]");
    	System.out.println("[horizontal="+scrollPanel.horizontal.getValue()+" vertical="+scrollPanel.vertical.getValue()+"]");
    	*/
    }
   ...
   void load(InferenceGraph inference_graph) {
    	ig = inference_graph;
	    repaint();
	    scrollPanel.updateScrollbars(true);
   }
